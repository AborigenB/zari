...
class AuthController extends Controller
{
    ...
    public function painters(Request $request)
    {
        $workName = $request->input('workname');

        if ($workName) {
            // Фильтруем пользователей по названию работы
            $users = User::whereHas('arts', function ($query) use ($workName) {
                $query->where('title', 'like', '%' . $workName . '%');
            })->get();
        } else {
            // Если нет запроса, выводим всех пользователей с работами
            $users = User::has('arts')->get();
        }

        return view('pages.auth.profile.variants.painters', compact('users', 'workName'));
    }
    ...
    public function showSocials($id)
    {
        ...
        return view('pages.auth.profile.variants.socials', compact('user', 'socials'));
    }

    public function showBasket($id)
    {
        $user = User::findOrFail($id);
        $arts = Art::whereHas('baskets', function ($query) use ($user) {
            $query->where('user_id', $user->id);
        })->get();
        return view('pages.auth.profile.variants.basket', compact('user', 'arts'));
    }
    public function showOrders($id)
    {
        $user = User::findOrFail($id);
        $orders = Order::where('user_id', $user->id)->with('orderItems.art')->get();
        return view('pages.auth.profile.variants.orders', compact('user', 'orders'));
    }
    public function showAdmin()
    {
        $users = User::all();
        // $arts = Art::all();
        // арты со статусом не опубликовано
        $unarts = Art::where('status', 'Не опубликовано')->get();
        $arts = Art::where('status', '!=', 'Не опубликовано')->get();
        $orders = Order::all();
        $questions = Question::all();

        $reviews = Review::all();
        return view('pages.admin.index', compact('users', 'arts', 'unarts', 'reviews', 'orders', 'questions'));
    }
    ...
    public function acceptArt($id)
    {
        $art = Art::findOrFail($id);
        $art->status = 'Опубликован';
        $art->save();
        return redirect()->back();
    }
